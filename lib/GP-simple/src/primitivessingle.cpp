...
// -----------------------------------------------------------------------------
PrimitivesSingle::PrimitivesSingle(): m_need_identity( false ),
...
// -----------------------------------------------------------------------------
std::pair<uint32, uint32> PrimitivesSingle::Find( const std::string& token )
{
...
// -----------------------------------------------------------------------------
void PrimitivesSingle::Register( uint32 arity, const std::string& name, const std::string& symbol,
...
// -----------------------------------------------------------------------------
void PrimitivesSingle::Load( unsigned x_dim, unsigned max_gen_size, const std::string& primitives )
{
   std::vector<std::vector<uint32> > per_arity;

   // Adding all problem variables (arity = 0)
   per_arity.resize(1);
   for( unsigned i = 0; i < x_dim; ++i )
      per_arity[0].push_back( PackNode( 0, GPT_VAR, (uint32)i ) );
   ...
   while( std::getline( ss, token, ',' ) )
   {
      ...
      if( token == "all" ) // mostly for testing purposes
      {
         for( unsigned i = 0; i < DB.size(); ++i )
         {
            std::pair<uint32, uint32> node = std::make_pair( DB[i].arity, i );
      ...
      {
         std::pair<uint32, uint32> node = Find( token );
...
// -----------------------------------------------------------------------------
uint32 PrimitivesSingle::RandomNode( unsigned min, unsigned max ) const
{
   ...
   uint32 node = m_primitives[Random::Int( m_primitives_boundaries[min].first,
   ...
   if( INDEX( node ) == GPT_EPHEMERAL )
      ...
      node = PackNode( 0, GPT_CLASS, (uint32) Random::Int( m_min_Y, m_max_Y ) );
