...
ParamsParallel* GPParallel::m_params = 0;

// -----------------------------------------------------------------------------
GPParallel::GPParallel( ParamsParallel& p, cl_device_type device_type ): m_device_type( device_type ),
                                                 m_X( 0 ),
                                                 m_E( 0 ),
                                                 m_num_points( 0 ),
                                                 m_y_dim( 1 ), 
...
// -----------------------------------------------------------------------------
void GPParallel::Evolve(void *out)
{
   /*
...
void GPParallel::Breed( cl_uint* old_pop, cl_uint* new_pop )
{
   // Elitism
...
// -----------------------------------------------------------------------------
void GPParallel::Crossover( const cl_uint* mom, const cl_uint* dad, cl_uint* child ) const
{
...
// -----------------------------------------------------------------------------
void GPParallel::SubTreeMutate( cl_uint* program ) const
{
   assert( program != NULL );
 ...
 */
void GPParallel::NeighborTerminalMutate( cl_uint& terminal ) const
{
   const cl_float precision = SCALE_FACTOR / (cl_float) COMPACT_RANGE;

   switch( INDEX( terminal ) )
   {
      case PrimitivesParallel::GPT_EPHEMERAL:
         if( Random::Probability( 0.5 ) ) // same probability for increment/decrement
         {
            if( AS_FLOAT( terminal ) < COMPACT_RANGE - precision )
               ...
               PrimitivesParallel::RepackNodeValue( terminal, AS_FLOAT( terminal ) + precision );
      ...
      case PrimitivesParallel::GPT_CLASS:
         if( Random::Probability( 0.5 ) ) // same probability for increment/decrement
         {
            if( AS_INT( terminal ) < m_primitives.m_max_Y )
               // increment a little bit
               PrimitivesParallel::RepackNodeValue( terminal, AS_INT( terminal ) + 1 );
         ...
         break;
      case PrimitivesParallel::GPT_VAR:
         if( Random::Probability( 0.5 ) ) // same probability for increment/decrement
         {
...
// -----------------------------------------------------------------------------
void GPParallel::NodeMutate( cl_uint* program ) const
{
   assert( program != NULL );
   assert( ProgramSize( program ) <= MaximumTreeSize() );
   assert( ProgramSize( program ) >= MinimumTreeSize() );
...
// -----------------------------------------------------------------------------
#if 0
void GPParallel::CopySubTreeMutate( const cl_uint* program_orig, cl_uint* program_dest ) const
{
...
// -----------------------------------------------------------------------------
void GPParallel::CopyNodeMutate( const cl_uint* program_orig, cl_uint* program_dest ) const
{
...
// -----------------------------------------------------------------------------
void GPParallel::Clone( const cl_uint* program_orig, cl_uint* program_dest ) const
{
...
bool GPParallel::EvaluatePopulation( const cl_uint* pop )
{
   KernelLaunch( pop );
...
// -----------------------------------------------------------------------------
void GPParallel::CalculateErrors( const cl_uint* pop )
{
   // Read the errors
...
// -----------------------------------------------------------------------------
bool GPParallel::KernelLaunch( const cl_uint* pop )
{
...
// -----------------------------------------------------------------------------
void GPParallel::UpdateBestProgram( const cl_uint* program, cl_float error )
{
...
unsigned GPParallel::Tournament( const cl_uint* pop ) const
{
...
// -----------------------------------------------------------------------------
void GPParallel::CreateBufferDataPoints()
{
...
// -----------------------------------------------------------------------------
void GPParallel::CreateBufferErrors()
{
...
// -----------------------------------------------------------------------------
void GPParallel::CreateBufferPopulation()
{
  /* 
...
void GPParallel::BuildKernel()
{
   ...
   std::string interpreter = "#define INTERPRETER_CORE" + (m_primitives.m_need_identity ? 
            " case " + util::ToString( (unsigned) PrimitivesParallel::GPF_IDENTITY ) + ": PUSH_1(ARG(0)) break;" : " ");

   for( unsigned i = 0; i < m_primitives.m_primitives.size(); ++i )
      if( INDEX( m_primitives.m_primitives[i] ) != PrimitivesParallel::GPT_VAR )
      {
...
// -----------------------------------------------------------------------------
void GPParallel::LoadKernel( const char* kernel_file )
{
...
//////////////////////////// Genetic Operations ///////////////////////////////
// -----------------------------------------------------------------------------
void GPParallel::InitializePopulation( cl_uint* pop )
{
...
// -----------------------------------------------------------------------------
void GPParallel::PrintProgram( const cl_uint* program ) const
{
   PrintTree( program + 1 );
}

// -----------------------------------------------------------------------------
void GPParallel::PrintProgramPretty( const cl_uint* program, int start, int end ) const
{
...
// -----------------------------------------------------------------------------
void GPParallel::PrintNode( const cl_uint* node ) const
{
   switch( INDEX( *node ) )
   {
      case PrimitivesParallel::GPT_VAR:
      ...
      case PrimitivesParallel::GPT_CLASS:
         ...
         break;
      case PrimitivesParallel::GPF_IDENTITY:
         std::cout << "I";
         break;
      default:
...
// -----------------------------------------------------------------------------
void GPParallel::PrintTree( const cl_uint* node ) const
{
   int sum = 0;
...
// -----------------------------------------------------------------------------
void GPParallel::CreateLinearTree( cl_uint* node, unsigned size ) const
{
   assert( size >= 1 );
   assert( node != 0 );
...
// -----------------------------------------------------------------------------
void GPParallel::LoadPoints( std::vector<std::vector<cl_float> > & out_x )
{
